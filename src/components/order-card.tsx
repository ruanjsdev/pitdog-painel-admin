...
export function OrderCard({ isSelected, onOpenDiscount, onSelect, order }: Props) {
  return (
    <button
      type="button"
      onClick={onSelect}
          ...
          <button
            ...
            className={`grid h-8 w-8 place-items-center rounded-lg border transition ${
              order.discount || order.discountPercent
                ? "border-orange-200/60 bg-orange-300 text-black"
                : "border-white/10 bg-black/20 text-orange-100 hover:bg-white/10"
            }`}
            aria-label={`Abrir desconto do pedido ${order.id}`}
          >
            <ReceiptText size={15} />
          </button>
